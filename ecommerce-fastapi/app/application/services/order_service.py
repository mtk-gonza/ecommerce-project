...
class OrderService:
    ...
    def create_order_from_cart(self, user_id: int, cart_id: int, 
    ...
    def create_order_direct(self, user_id: int, items: List[Dict[str, Any]],
                          shipping_address: str, billing_address: str,
                          notes: Optional[str] = None) -> Order:
        """
    ...
    def cancel_order(self, order_id: int, reason: Optional[str] = None,
        ...
        if not order.can_be_cancelled:
            raise OrderAlreadyShippedException(
                f"No se puede cancelar una orden en estado '{order.status.value}'"
            )
    ...
    def refund_order(self, order_id: int, amount: Optional[Decimal] = None,
                    commented_by: Optional[int] = None) -> Order:
